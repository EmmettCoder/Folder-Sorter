...
public class SorterLogic {
        ...
        for (File f : files) {
            if (!f.isDirectory()) { // Checks if the current file is a folder. If it is, skip.
                ...
                if (fileSplit.length > 1) { // If the file name has an extension. If not, skip.
                    String destFolder = co.getFolderName(fileSplit[fileSplit.length - 1]); // Get the folder for the
                                                                                           // file's extension.
                    ...
                    if (!(destFolder == null)) { // If there is not a folder for the extension, do nothing.
                        ...
                        try {
                        ...
                        } catch (FileAlreadyExistsException faee) { // If the file already exists, add a 1 to the end of
                                                                    // the filename.
                            ...
                            if (lastOpenParen != -1 && lastCloseParen != -1 && lastCloseParen > lastOpenParen) {
                                try {
                                    String numStr = baseName.substring(lastOpenParen + 1, lastCloseParen).trim(); // Extract
                                                                                                                  // the
                                                                                                                  // string
                                                                                                                  // between
                                                                                                                  // (
                                    ...
                                    String sortBaseName = baseName.substring(0, lastOpenParen).trim(); // Strip the old
                                                                                                       // (n) from the
                                                                                                       // base name
                                    ...
                                    destination = Paths.get(destBeginning +
                                            ...
                                            File.separator +
                                            newName);
                                } catch (NumberFormatException e) { // If the stuff in () wasn't actually a number,
                                ...
                                destination = Paths
                                        .get(destBeginning + File.separator + destFolder + File.separator + baseName
                                                + " (1)." + extension);
                            }
    ...
    public static boolean sortThereSimilar(String directory, int similarityThreshold) {
        ...
        // Remove single files. Making a new map and deleting the old one. Cannot edit
        // while iterating.
        Map<String, ArrayList<File>> likenessNoDupes = new HashMap<>();
        for (String threshString : likeness.keySet()) {
            if (likeness.get(threshString).size() > 1)
                likenessNoDupes.put(threshString, likeness.get(threshString));
        }
    ...
    private static boolean makeFolders(String directory) {
        Collection<String> foldersList = co.getAllFolderNames(); // Get all the folder names and their correlated
                                                                 // extension.
        Path basePath = Paths.get(directory);

        for (String currFolder : foldersList) { // Iterate over each folder.
            Path relativePath = Paths.get(currFolder); // Using relativePath and currWalkPath helps the program know
                                                       // which was made
            ...
            for (int i = 0; i < relativePath.getNameCount(); i++) { // getNameCount() gets the num of each individual
                                                                    // subdirectory.
                currWalkPath = currWalkPath.resolve(relativePath.getName(i));
                try {
                    if (Files.notExists(currWalkPath)) { // If the folder was not already there, add it to the new
                                                         // folders list.
